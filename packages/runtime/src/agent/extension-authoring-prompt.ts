...
export function buildExtensionAuthoringNote(agentDir: string): string {
  ...
  const lines = [
    ...
    "- Do NOT use ctx.ui.custom(), TUI chrome (setFooter/setHeader/setEditorComponent/onTerminalInput), tool renderCall/renderResult, registerShortcut, or registerMessageRenderer: they load but silently do nothing in Yui. registerCommand handlers do run via the slash menu, but their session-control actions (newSession/fork/navigateTree/switchSession) resolve to { cancelled: true } — Yui owns session navigation; ctx.waitForIdle() and ctx.reload() work.",
